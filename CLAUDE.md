...
Não há linter configurado. Auto-teste da lógica pura sem Xcode: `swift build -c release && ./.build/release/Pynkaro --selftest`. O app só roda de fato em macOS (frameworks Speech/AVFoundation/AppKit) e requer permissões de Microfone e Reconhecimento de Fala concedidas ao terminal na primeira execução.
...
O centro é a máquina de estados em `VoiceAssistant.swift`: `waitingWakeWord → capturingQuestion → thinking → speaking → waitingWakeWord`. Tudo converge para ela; os demais arquivos são satélites plugados por closures. `SelfTest.swift` roda a suíte de auto-teste com `--selftest`.
